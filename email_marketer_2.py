...
def append_website_urls_from_gemini_response(urls):
    try:
        response = gemini_model.generate_content("websites which has email marketing , print each of them in url format and on next line and don't print anything else")
        for line in response.text.strip().splitlines():
            if validators.url(line):
                urls.append(line)
    except Exception as e:
        print(f"Failed to append website URLs from Gemini API response: {e}")

# Call the function to append website URLs from Gemini API response
append_website_urls_from_gemini_response(urls)
print(urls)
...
save_data_to_excel(data_list, 'extracted_data_with_gemini_api_from_descriptions_self_fetched.xlsx')

print("Data has been extracted and saved to extracted_data_with_gemini_api_from_descriptions_self_fetched.xlsx")
